...
(async () => {
  ...
  if (!run) { console.log('не дождались'); process.exit(1); }
  console.log('статус:', run.conclusion, run.html_url);
  ...
  if (fs.existsSync(f)) {
    ...
    const lines = t.split(/\r?\n/).filter(l => /widget|готово|ipa$|ipa\s|error/i.test(l) && !/warning/i.test(l));
